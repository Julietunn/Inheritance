...
//									Get/Set
	...
	tm get_birty_date()const
	{
		return birth_date;
	}
	unsigned int get_age()const
	{
		time_t t_today = time(NULL); //Получаем текущую дату/время в формате Timestamp
		//cout << today << endl;
...
public:
	...
	Student(const Human& human, const std::string& speciality, const std::string& group, double rating, double attendance):Human(human)
	{
		set_speciality(speciality);
		set_group(group);
		set_rating(rating);
...
public:
	...
	//									Constructor
	Teacher(HUMAN_TAKE_PARAMETERS, TEACHER_TAKE_PARAMENERS) :Human(HUMAN_GIVE_PARAMETERS)
	{
		set_speciality(speciality);
...
{
	for (int i = 0; i < n / sizeof(group[0]); i++)
	{
		//group[i]->info();
		cout << *group[i] << endl;
		cout << "\n--------------------------------------------\n";
